...
int main(void)
{
	...
	for (int i = 0; i < CONFIG_DMA_TRANSFER_CHANNEL; i++) {
		...
		LOG_INF("Channel %d: Initialized %d bytes across %d SG blocks",
			i, TOTAL_BUFFER_SIZE, NUM_SG_BLOCKS);
#else
	...
	for (int i = 0; i < CONFIG_DMA_TRANSFER_CHANNEL; i++) {
		...
		for (int j = 0; j < NUM_SG_BLOCKS; j++) {
			if (memcmp(&src_buffer[i][j * BUFFER_SIZE],
				&dst_buffer[i][j * BUFFER_SIZE],
				BUFFER_SIZE) != 0) {
