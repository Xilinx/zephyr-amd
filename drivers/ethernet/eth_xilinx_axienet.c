...
static int xilinx_axienet_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	/* Do not send if interface is not started */
	if (!data->started) {
		LOG_WRN("Attempted to send while interface is down");
		return -ENETDOWN;
	}
...
static int xilinx_axienet_stop(const struct device *dev, struct net_if *iface __unused)
{
	...
	if (!data->started) {
		LOG_INF("%s: already stopped, nothing to do", dev->name);
		return 0;
	}
...
static int xilinx_axienet_start(const struct device *dev, struct net_if *iface __unused)
{
	...
	if (data->started) {
		LOG_INF("%s: already started, nothing to do", dev->name);
		return 0;
	}
	...
	if (!data->tx_buffer) {
		LOG_ERR("%s: failed to allocate %zu TX buffers",
			dev->name, data->num_tx_buffers);
		return -ENOMEM;
	}
	...
	if (!data->rx_buffer) {
		LOG_ERR("%s: failed to allocate %zu RX buffers",
			dev->name, data->num_rx_buffers);
		k_free(data->tx_buffer);
		data->tx_buffer = NULL;
		return -ENOMEM;
	}
